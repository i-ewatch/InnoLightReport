...
{
    public partial class VoltageDayReportUserControl : Field4UserControl
    {
        #region 基本資訊
        /// <summary>
        /// 跳出迴圈旗標
        ...
        #endregion
        public VoltageDayReportUserControl(MssqlMethod method, DeviceSetting device)
        {
            InitializeComponent();
            ...
            {
                foreach (var Systemitem in DeviceSetting.VoltageNames)
                {
        ...
        {
            sql += $"DECLARE @Kwh{i} AS TABLE ([Time] nvarchar(10), [Name] nvarchar(50), [Min] DECIMAL(18,2), [Max] DECIMAL(18,2), [Avg] DECIMAL(18,2))  " +
                   $"INSERT INTO @Kwh{i}  ([Time], [Name],[Min],[Max],[Avg]) " +
                   $"SELECT  convert(varchar(10),[Timestamp], 120)AS[Time], MAX(Name)AS[Name],MIN(CAST([Value] AS DECIMAL(18,2)))AS[Min],MAX(CAST([Value] AS DECIMAL(18,2)))AS[Max],AVG(CAST([Value] AS DECIMAL(18,2)))AS [Avg] FROM [dbo].[Table_V] " +
        ...
        private string SelectFunction(int Index, string sql)
        {
            ...
            for (int i = 0; i < Index; i++)
            {
                sql += $" LEFT JOIN @Kwh{i} AS Kwh{i} ON  convert(varchar(10),mainT.[ttimen],120) = Kwh{i}.[Time]";
            }
            return sql;
        }
    }
