...
namespace InnoLightReport.Views
{
    public partial class KwhHourReportXtraUserControl : Field4UserControl
    {
        ...
        {
            sql += $"DECLARE @Kwh{i} AS TABLE ([Time] nvarchar(13), [Name] nvarchar(20), [Total] DECIMAL(18,2)) " +
                   $"INSERT INTO @Kwh{i}  ([Time], [Name],[Total]) " +
                   $"SELECT  convert(varchar(13),[TIMESTAMP], 120)AS[Time], MAX(NAME)AS[Name],(MAX(CAST([VALUE] AS DECIMAL(18,2))) - MIN(CAST([VALUE] AS DECIMAL(18,2)))) AS Total FROM [dbo].[PM] " +
                   $"WHERE NAME = 'PM.{item.TagName}.KWH' " +
