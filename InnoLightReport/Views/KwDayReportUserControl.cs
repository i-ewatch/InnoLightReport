...
{
    public partial class KwDayReportUserControl : Field4UserControl
    {
        #region 基本資訊
        /// <summary>
        /// 跳出迴圈旗標
        ...
        #endregion
        public KwDayReportUserControl(MssqlMethod method, DeviceSetting device)
        {
            InitializeComponent();
        ...
        private string Select_Current(int i, string sql, DeviceName item, DateTime StartTime, DateTime EndTime)
        {
            sql += $"DECLARE @Kwh{i} AS TABLE ([Time] nvarchar(10), [Name] nvarchar(50), [Min] DECIMAL(18,2), [Max] DECIMAL(18,2), [Avg] DECIMAL(18,2))  " +
                   ...
                   $"SELECT  convert(varchar(10),[Timestamp], 120)AS[Time], MAX(Name)AS[Name],MIN(CAST([Value] AS DECIMAL(18,2)))AS[Min],MAX(CAST([Value] AS DECIMAL(18,2)))AS[Max],AVG(CAST([Value] AS DECIMAL(18,2)))AS [Avg] FROM [dbo].[Table_KW] " +
